...
urlpatterns = [
	...
	url(_(r'^(?P<challenge_slug>[-\w]+)/solution/create$'), SolutionCreateView.as_view(), name="challenge_solution_create"),
	url(_(r'^(?P<challenge_slug>[-\w]+)/solution/(?P<pk>[0-9]+)/update$'), SolutionUpdateView.as_view(), name="challenge_solution_update"),
	url(_(r'^(?P<challenge_slug>[-\w]+)/solution/(?P<pk>[0-9]+)/remove$'), SolutionDeleteView.as_view(), name="challenge_solution_delete"),
]