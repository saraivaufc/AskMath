...
class QuestionDetailView(SingleObjectMixin, FormView):
	template_name = 'courses/question/detail.html'
	...
	def get_success_url(self):
		return reverse_lazy('courses:answer_question', kwargs={'course_slug': self.get_course().slug, 'lesson_slug': self.get_lesson().slug})
	...
	def get(self, request, * args, ** kwargs):
		...
		if not self.object and self.get_lesson().questions.all():
			return HttpResponseRedirect(reverse_lazy('courses:lesson_finished', kwargs={'course_slug': self.get_course().slug, 'slug': self.get_lesson().slug } ))
